#
# Use the following PowerShell script to set O365 mailbox forwarding to NULL.  Once an attacker
# has compromised an O365 account, sometimes they will setup forwarding to receive a copy
# of the victims e-mail.  If you find a "funny" looking forward, use this script to clear the forward.
# 
# Dependiences:
...
If (($Session -ne $NULL) -or ($UserCredential -ne $NULL)) {
	Do {
		$email = accountInput "Clear Account Forwarding"
	
		if ($email -ne "quit") {
		
			# set forwarding to null
		    Get-Mailbox $email -ResultSize 1 | Set-Mailbox -ForwardingSmtpAddress $NULL -DeliverToMailboxAndForward $false
